...
function Header({ onToggleMobileMenu }) {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchTerm.trim()) {
      // Navega para a página de pesquisa, passando o termo na URL
    ...
    <header className="bg-white h-20 shadow-md fixed top-0 left-0 md:left-64 right-0 z-30">
      <div className="container mx-auto px-4 sm:px-6 h-full flex items-center justify-between">
        ...
        <div className="md:hidden">
          {" "}
          {/* Este div garante que o botão só apareça em mobile */}
          <button
        ...
        <form
          ...
          className="relative flex-grow max-w-xs sm:max-w-xl ml-2 md:ml-0"
        >
          <input
            type="text"
            ...
            className="w-full py-2.5 pl-10 pr-4 border border-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0D334D] focus:border-transparent transition-colors"
          />
          <MagnifyingGlassIcon className="h-5 w-5 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2" />
        </form>

        {/* Botões de Ação (Login/Cadastro ou Perfil/nova categoria) */}
